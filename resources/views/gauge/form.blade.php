    ...
    <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <x-input disabled type="text" name="name" class="mt-1 block w-full bg-gray-100" value="{{ $gauge->name }}" />
                <x-input-error for="name" class="mt-2" />
            </div>
            <div>
                <x-label for="title" value="{{ __('Title') }} *" />
                <x-chimera::multi-lang-input id="title" name="title" value="{{ old('title', $gauge->title ?? null) }}" />
                <x-input-error for="title" class="mt-2" />
            </div>
            <div>
                <x-label for="subtitle" value="{{ __('Sub-title') }} *" />
                <x-chimera::multi-lang-input id="subtitle" name="subtitle" value="{{ old('subtitle', $gauge->subtitle ?? null) }}" />
                <x-input-error for="subtitle" class="mt-2" />
            </div>
            <div>
                <x-label for="rank" value="{{ __('Rank') }}" />
                <x-input id="rank" name="rank" type="number" class="w-20 mt-1" value="{{ old('rank', $gauge->rank) }}" />
            ...
            <div>
                ...
                <select name="inapplicable_levels[]" multiple class="mt-1 space-y-1 text-base p-1 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                    ...
                    @foreach($areaHierarchies as $id => $name)
                        <option class="p-2 rounded-md" value="{{ $id }}" @selected(in_array($id, $gauge->inapplicableLevels->pluck('id')->all()))>{{ $name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="flex items-center mt-3 ml-3" x-data="{enabled: @json($gauge->published ?? false) }">
                    <label for="status">
