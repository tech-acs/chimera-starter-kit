        ...
        <div class="p-2 pb-4 md:flex md:space-x-4 lg:space-x-8 xl:space-x-10 items-baseline space-y-4 md:space-y-2" x-data="{ loading: false }">
            ...
            @foreach ($dropdowns as $levelName => $dropdown)
                <div class="flex items-baseline space-x-4">
                    ...
                    <div class="relative">
                        <select id="{{ $levelName }}" name="{{ $levelName }}" wire:change="changeHandler('{{ $levelName }}', $event.target.value);" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                            ...
                            @foreach($dropdown['list'] as $path => $areaName)
                                <option value="{{ $path }}" @selected($path === ($dropdown['selected'] ?? null))>{{ $areaName }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
