...
#[Description('Create a new map indicator (colored map boundaries) artefact. Generates a map indicator class file from a stub and creates the database record.')]
class CreateMapIndicator extends Tool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'name' => $schema->string(
                description: 'Component name in CamelCase (e.g., "PopulationDensity" or "Thematic/PovertyIndex")',
            ),
            'title' => $schema->string(
                description: 'Human-readable title',
            ),
            'data_source' => $schema->string(
                description: 'Name of the data source this map indicator queries',
            ),
