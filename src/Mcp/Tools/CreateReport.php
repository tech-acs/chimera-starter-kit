...
#[Description('Create a new report (Excel export) artefact. Generates a Livewire component file from a stub and creates the database record. Prerequisites: call list-data-sources first and ask the user which data source to use, then parse the dictionary with parse-dictionary. After creation, implement getData() by editing the file at app/Reports/{Name}.php.')]
class CreateReport extends Tool
{
    public function handle(Request $request, CreateArtefactAction $createArtefactAction): Response
    {
        ...
        if ($result->success) {
            return Response::text('Report created successfully');
        }
    ...
    {
        return [
            'name' => $schema->string()->description('Component name in CamelCase, including directory if any (e.g., "EnumeratorList", "Region/CompletionReport")'),
