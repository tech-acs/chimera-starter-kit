...
class EditMapIndicator extends Tool
{
    public function handle(Request $request): Response
    {
        ...
        $mapIndicator = MapIndicator::find($id);

        if (! $mapIndicator) {
            return Response::error("Map indicator with ID {$id} not found.");
        }
        ...
        return Response::text("Map indicator '{$mapIndicator->title}' (ID: {$id}) updated.");
    }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'id' => $schema->integer(
                description: 'ID of the map indicator to update',
            ),
            'title' => $schema->string(
