<x-app-layout>
    ...
    <div class="flex flex-col max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
        <div class="text-right">
            <a href="{{route('manage.faq.create')}}"><x-jet-button>{{ __('Create new') }}</x-jet-button></a>
        </div>
        @if (session('message'))
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            @forelse($records as $record)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 leading-6 overflow-clip">
                                        <b>{{$record->question}}</b><br>
                                        {{Str::limit($record->answer, 120)}}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{route('manage.faq.edit', $record->id)}}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                        <span class="text-gray-400 px-1">|</span>
                                        <form action="{{route('manage.faq.destroy', $record->id)}}" method="post" class="inline">
                                            @method('delete')
                                            @csrf
