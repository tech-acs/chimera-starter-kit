...
function ChartEditor({dataSources, initialData, initialLayout, config, indicatorId, indicatorTitle, defaultLayout}) {
    ...
    const save = async () => {
        ...
        const response = await axios.post(`/api/indicator/${indicatorId}`, { data, layout }, {validateStatus: () => true});
    ...
    return (
        <>
            <div className="flex justify-between w-full bg-gray-50">
                <div className="text-gray-600 text-xl font-medium items-center p-3 w-1/2 truncate">{indicatorTitle}</div>
                <div className="flex justify-end gap-x-4 p-3">
                    <div className="flex items-center font-medium mr-8"
